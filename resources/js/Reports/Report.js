...
app.config(['$routeProvider', function($routeProvider) {
    ...
    project_name=appPath;
	  ...
	  $routeProvider.when('/sale_summary_date', {
		templateUrl : '/'+project_name+'/sale_summary_date',
		controller : 'rptCtrl'

	}).when('/AttDep', {
		templateUrl : '/'+project_name+'/AttDep',
		controller : 'rptCtrl'

	}).when('/AttEmp', {
		templateUrl : '/'+project_name+'/AttEmp',
		controller : 'rptCtrl'

	}).when('/AttDesig', {
		templateUrl : '/'+project_name+'/AttDesig',
		controller : 'rptCtrl'

	}).when('/AttAll', {
		templateUrl : '/'+project_name+'/AttAll',
		controller : 'rptCtrl'

	}).when('/AttSum', {
		templateUrl : '/'+project_name+'/AttSum',
		controller : 'rptCtrl'
	}).when('/sale_TA', {
		templateUrl : '/'+project_name+'/sale_TA',
		controller : 'rptCtrl'

	}).when('/sale_PC', {
		templateUrl : '/'+project_name+'/sale_PC',
		controller : 'rptCtrl'

	}).when('/spw', {
		templateUrl : '/'+project_name+'/Stock_Product',
		controller : 'rptCtrl'

	}).when('/saleproduct', {
		templateUrl : '/'+project_name+'/Sale_Product',
		controller : 'rptCtrl'

	}).when('/scw', {
		templateUrl : '/'+project_name+'/Stock_Category',
		controller : 'rptCtrl'

	}).when('/Main', {
		templateUrl : '/'+project_name+'/POSMain',
		controller : 'abcCtrl'
...
app.controller('abcCtrl', function($scope, $http, $filter,$location,$rootScope){
	

	$scope.locate = function(){ 
	
window.location.href = "#/AttDep";
...
app.controller('rptCtrl', function($scope, $http, $filter,$location,$rootScope, $sce) {
	...
	$scope.getSaleInvoiceListReport = function(id){
		...
		if(isEmpty($scope.ReportDTO.site))
		{
		$scope.ReportDTO.site='';
		}
		...
		else  if(id==='emp')
		{
				...
				$scope.ReportDTO.txt_report_type='emp';
		}
		else  if(id==='sum')
		{
		...
		{
				Report_name='attendance_all.jasper';
				$scope.ReportDTO.txt_report_type='all';
		}
		else  if(id==='desig')
		{
				Report_name='attendance_desig.jasper';
				$scope.ReportDTO.txt_report_type='desig';
		}
...
$scope.generateAttendenceReportExcel = function(id){
...
//						"&Report_name="+Report_name,{responseType: 'arraybuffer' }).success(function(data) {
		  ...
		  $http.get('/'+project_name+'/generateAttendenceReportExcel?dte_date_from='+$scope.ReportDTO.dte_date_from+
				  ...
				  '&product='+$scope.ReportDTO.txt_product_name+
				  '&Status='+$scope.ReportDTO.txtStatus+
				  '&Report_name='+$scope.ReportDTO.Report_name,{responseType: 'arraybuffer' }).success(function(data) {
