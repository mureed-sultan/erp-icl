...
app.controller('customerCtrl', function($scope, $http, $filter){
	...
	$scope.getCustomer = function(){
        ...
        $http.get('/'+project_name+'/getCustomerActiveUser').success(function(data) {
...
$http.get('/'+project_name+'/getAllIncoTerm').success(function(data) {
        ...
        $http.get('/'+project_name+'/getAllArea').success(function(data) {
		 ...
		 $http.get('/'+project_name+'/getAllDealer').success(function(data) {
			 debugger;
	...
	$scope.addCustomer = function (){
		...
		$scope.newCfgCustomerSetup.blIsDealer=true;
		...
		$.ajax({
            ...
            success: function (data) {
            	if(data=='Failure'){
            		$('#errMsgText').html("Unable to add Dealer");
            		$('#addErrorAlert').show();
            	}
            	else if(data=='Success'){
            		$('#successMsgText').html("Dealer added successfully");
            		$('#successAlert').show();
            		$scope.newCfgCustomerSetup = {};
            		$scope.init();
            	}
            	else if(data=='error'){
            		$('#errMsgText').html("Unable to add Dealer \n due to duplication of  Dealer");
            		$('#addErrorAlert').show();
            	}
            },
	...
	$scope.updateCustomer = function(){
		...
		$.ajax({
			...
			success: function (data) {
            	if(data=='Failure'){
            		$('#errMsgText').html("Unable to edit Dealer");
            		$('#addErrorAlert').show();
            	}
            	else if(data=='Success'){
            		$('#successMsgText').html("Dealer edit successfully");
            		$('#successAlert').show();
            		$scope.init();
            	}
  	  ...
  	  $.each(dataTable, function(index, customer) {
  		  var editDeleteColumn ='<div class="dropdown" id="dropdown'+customer.serCustomerId+'">'+
									'<a href="" data-toggle="dropdown" onclick="angular.element(this).scope().populateEditDialog('+customer.serCustomerId+');angular.element(this).scope().$apply();" ><i class="glyphicon glyphicon-pencil"></i>'+
							  		...
							  		'<li><a href="" data-toggle="modal" onclick="angular.element(this).scope().populateEditDialog('+customer.serCustomerId+');angular.element(this).scope().$apply();" data-target="#editCustomerForm">Edit Dealer</a></li>'+
							  		  '</ul>';
...
//	    	  customer.txtCustomerCode,
	    	  ...
	    	  customer.txtCustomerCode,
	    	  isEmpty(customer.cfgTblCustomerCategory)?"":customer.cfgTblCustomerCategory.txtCustomerCategoryName,
	    			  customer.txtPhoneNo,
